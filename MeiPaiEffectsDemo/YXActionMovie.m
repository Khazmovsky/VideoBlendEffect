...
//#import "YXActionMovieEffectGenerator.h"

@interface YXActionMovie () <AVPlayerItemOutputPullDelegate>
{
...
//    if (soundTracks.count) {
//        self.audioPlayer = [[YXAudioQueuePlayer alloc] initWithAudioPath:self.url.path];
//    } else {
//        NSString *themeSoundPath = [[NSBundle mainBundle] pathForResource:[[[self.url.pathComponents lastObject] componentsSeparatedByString:@"_"][0] stringByAppendingString:@"_sound"] ofType:@"caf"];
//        self.audioPlayer = [[YXAudioQueuePlayer alloc] initWithAudioPath:themeSoundPath];
//    }
}
...
{
    _isProcessing = YES;
//    if (self.audioPlayer && self.playAtActualSpeed) {
...
//    }
    ...
    __weak YXActionMovie __block *blockSelf = self;
...
- (void)processAsset
{
    ...
    __unsafe_unretained YXActionMovie *weakSelf = self;
        ...
        if (reader.status == AVAssetWriterStatusCompleted) {
            ...
            if (keepLooping) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    if (self.yx_delegate && [self.yx_delegate respondsToSelector:@selector(acitonMovieReplayed:)]) {
                        [self.yx_delegate acitonMovieReplayed:self];
                    }
                });
            } else {
...
- (void)processPlayerItem
{
    ...
    [self.playerItem addOutput:playerItemOutput];
    [playerItemOutput requestNotificationOfMediaDataChangeWithAdvanceInterval:.1f];
...
- (void)displayLinkCallback:(CADisplayLink *)sender
{
	...
	if ([playerItemOutput hasNewPixelBufferForItemTime:outputItemTime]) {
        ...
        if (self.yx_delegate && [self.yx_delegate respondsToSelector:@selector(actionMovie:currentFrameTime:)]) {
            [self.yx_delegate actionMovie:self currentFrameTime:CMTimeGetSeconds(outputItemTime)];
        }
	}
}
...
- (BOOL)readNextVideoFrameFromOutput:(AVAssetReaderOutput *)readerVideoTrackOutput;
{
    ...
    } else if (audioEncodingIsFinished && videoEncodingIsFinished) {
...
- (void)processMovieFrame:(CMSampleBufferRef)movieSampleBuffer;
{
    ...
    if (self.yx_delegate && [self.yx_delegate respondsToSelector:@selector(actionMovie:currentFrameTime:)]) {
        [self.yx_delegate actionMovie:self currentFrameTime:CMTimeGetSeconds(currentSampleTime)];
    }
...
{
    /*
    uint8_t *out_frame = malloc(sizeof(movieFrame));
    CVPixelBufferLockBaseAddress(movieFrame, 0);
    filter_bf_topmov((uint8_t *)CVPixelBufferGetBaseAddress(movieFrame), out_frame, 1, 1, 1, 1);
    CVPixelBufferUnlockBaseAddress(movieFrame, 0);
    
    
    CVPixelBufferRef new_pixelBuffer = NULL;
    
    CVPixelBufferCreateWithBytes(kCFAllocatorDefault, CVPixelBufferGetWidth(movieFrame), CVPixelBufferGetHeight(movieFrame), kCVPixelFormatType_32ARGB, out_frame, CVPixelBufferGetBytesPerRow(movieFrame), NULL, NULL, NULL, &new_pixelBuffer);
    
    movieFrame = new_pixelBuffer;
    */
...
- (void)endProcessing
{
    ...
    for (id<GPUImageInput> currentTarget in targets)
    {
        //防止mask 和 alpha video 结束时去结束writer
//        if ([currentTarget isMemberOfClass:[YXActionMovieMakeAlphaChannelFilter class]] && !_shouldFinishingRecordingWhenProcessingDone) {
//            continue;
//        }
    ...
    if (self.yx_delegate && [self.yx_delegate respondsToSelector:@selector(didCompletePlayingMovie:)] && _isProcessing) {
        [self.yx_delegate didCompletePlayingMovie:self];
    }
    _isProcessing = NO;
...
- (void)enableSynchronizedEncodingWithWriter:(GPUImageMovieWriter *)writer {
    synchronizedMovieWriter = writer;
}

- (void)addTarget:(id<GPUImageInput>)newTarget {
    [super addTarget:newTarget];
    if ([newTarget respondsToSelector:@selector(actionMovie:currentFrameTime:)]) {
        self.yx_delegate = (id<YXActionMovieDelegate>)newTarget;
    }
}
